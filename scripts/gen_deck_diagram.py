"""Generate the richer, presentation-quality architecture diagram for the slide deck.

Run: python scripts/gen_deck_diagram.py
Writes: docs/assets/inclusify-architecture-deck.png (committed; NOT served by the app --
see scripts/gen_architecture.py for the API-served diagram, which stays spec-§C-minimal.
This one is the fuller picture for slides: the EvidenceInvestigator's tool loop, the
provider panels, and a legend.)

Same rendering approach as gen_architecture.py, copied rather than imported (this script
must not touch that one, and its column-math module state is specific to its own smaller
diagram): Pillow, draw at SS=4x, LANCZOS downsample for free anti-aliasing, rounded
rectangles, filled arrowheads. Fixed pixel layout -- no timestamps, no randomness,
byte-identical across runs (see __main__ self-check).

Module names ([2]/[3]/[4] boxes) are pulled from MODULE_BY_TASK -- the same single
source of truth gen_architecture.py uses -- so this diagram can never drift from the
`steps[].module` log.

Layout strategy: a top-down "flow" cursor. Each band's height is derived from its own
wrapped content (measured at a fixed width) rather than guessed and clipped, so labels
never overflow their boxes -- `_fits_or_raise` is the hard backstop for that guarantee.
"""
...
OUT_PATH = (
    Path(__file__).resolve().parents[1] / "docs" / "assets" / "inclusify-architecture-deck.png"
)
